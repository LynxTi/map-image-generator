...
import { createPaperTextureSvg } from './common/paper.js';
import { createStandaloneOverlaySvg } from './common/overlay.js';
import { exportWebP } from './common/webp.js';
...
export async function renderVectorMap({ lat, lon, fileName }) {
  ...
  const paperTexture = createPaperTextureSvg(WIDTH, HEIGHT, `${fileName}|variant1`);
  const overlay = createStandaloneOverlaySvg(WIDTH, HEIGHT, markerX, markerY);
...
`;
  ...
  const outPath = path.resolve('out/variant1', `${fileName}.webp`);
  await exportWebP(svg, overlayBuffer, outPath, WIDTH, HEIGHT);
  return outPath;
}
